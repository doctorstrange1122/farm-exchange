...
KEYSTORE="$OUT_DIR/farm_v1.keystore"
KEYSTORE_PASS="farm2026"
KEY_ALIAS="farm_v1"
KEY_PASS="farm2026"
...
javac -source 1.8 -target 1.8 \
    ...
    "$SRC"/java/com/farm/*.java "$OUT_DIR"/gen/com/farm/R.java 2>/dev/null || \
javac -source 1.8 -target 1.8 \
    -bootclasspath /usr/lib/jvm/java-8-openjdk-amd64/jre/lib/rt.jar \
    -cp "$ANDROID_JAR:$ZXING_JAR" \
    -d "$OUT_DIR/java_classes" \
    "$SRC"/java/com/farm/*.java
echo "       编译完成"
...
if [ ! -f "$KEYSTORE" ]; then
    keytool -genkey -v \
        ...
        -dname "CN=FarmExchange, OU=Dev, O=Farm, L=Shenzhen, ST=Guangdong, C=CN"
fi
...
echo ""
echo "========================================"
echo "  APK: $OUT_DIR/farm_exchange.apk"
echo "  大小: $(ls -la "$OUT_DIR/farm_exchange.apk" | awk '{print $5}') bytes"
echo "========================================"
